...
import trafficSim.utils.StringList;
...
//	private IronAgent ironAgent;
	 ...
	 * Constructor
	 */
	public NormSetsFileManager(TrafficMetrics metrics, boolean convergence) {
...
//		this.metrics = metrics;
	...
	public void load() {
		...
		if(this.convergence){
			finalNormSetsFilename = TrafficConfiguration.SIM_DATA_PATH + "/Converged" + TrafficConfiguration.SIM_FINAL_NORMSETS_FILE;
			finalNormSetsPlotFilename = TrafficConfiguration.SIM_DATA_PATH + "/Converged" + TrafficConfiguration.SIM_FINAL_NORMSETS_FILE + ".plot";
		}
		else {
			finalNormSetsFilename = TrafficConfiguration.SIM_DATA_PATH + "/NotConverged" + TrafficConfiguration.SIM_FINAL_NORMSETS_FILE;
			finalNormSetsPlotFilename = TrafficConfiguration.SIM_DATA_PATH + "/NotConverged" + TrafficConfiguration.SIM_FINAL_NORMSETS_FILE + ".plot";
		}
		...
		this.loadNormsFile(finalNormSetsFile, finalNormSets, effNormSets, necNormSets, covNormSets);
	}

	/**
	 * Loads the norms from the norms file
	 */
	private void loadNormsFile(File file, HashMap<StringList, Integer> nsMap, HashMap<StringList, 
			Float> effMap, HashMap<StringList, Float> necMap, HashMap<StringList, Integer> covMap) {
...
//		StringList currNormSet = new StringList(ironAgent.getNormativeSystem().getNorms());
...
////				this.covNormSets.put(normSet, metrics.getCurrentNormSetCoverage());
//			}
//		}
//		
//		if(!exists) {
...
////			this.covNormSets.put(currNormSet, metrics.getCurrentNormSetCoverage());
//		}
	}
}
